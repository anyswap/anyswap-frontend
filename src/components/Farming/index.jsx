...
export default function Farming ({
  ...
  const useToken = INITIAL_TOKENS_CONTEXT[CHAINID]
  let exchangeObj = {}
  ...
  function farmsList () {
    if (lpArr.length <= 0 || config.symbol === 'HT') {
      ...
      return (
        <>
          <FarmListBox>
            {initPairs.map((item, index) => {
              return <FarmList key={index}>
                <FarmListCont>
                  <DoubleLogo>
                    <div className="logo left">
                      <TokenLogo1 address={item} size='100%'/>
                      </div>
                    ...
                    <Flex>
                      {
                        account ? (
                          <Button1 style={{height: '45px', maxWidth: '200px'}} onClick={() => {
                            ...
                            history.push(FARMURL + '/' + coin + '-' + useChain.symbol)
  ...
  function getPoolBaseBalance (exchangeETHBalance, totalSupply, lpBalance, exchangeTokenBalancem) {
    if (
      ...
      if (userInfo && userInfo.gt(ethers.constants.Zero)) {
        ...
        let pecent = userInfo.mul(ethers.utils.bigNumberify(1000000)).div(lpBalance)
        pecent = Number(pecent.toString()) / 10000
        ...
        <StakingBox>
          <StakingList>
            ...
            <StakingLi>
              ...
              <div className='content'>
                <h2 className='title'>{t('MyStaking')}({pbaObj.pecent ? formatNum(pbaObj.pecent) : '0.00'} %)</h2>
                <h3 className='num'>
