...
function CreateExchange({ location, params }) {
  ...
  // const { name, symbol, decimals, exchangeAddress } = useTokenDetails(tokenAddress.address ? tokenAddress.address : '')
  ...
  async function createExchange() {
    if (config.supportWallet.includes(walletType)) {
      ...
      getWeb3BaseInfo(FACTORY_ADDRESSES[chainId], data, account).then(res => {
        // console.log(res)
        if (res.msg === 'Success') {
