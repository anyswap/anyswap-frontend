...
export default function HTfaring({ initialTrade }) {
  ...
  return (
    <>
      <Modal style={{ userSelect: 'none' }} isOpen={showPopup} minHeight={null} maxHeight={90}>
        <ModalContent onClose={setPopup} isShowClose={!!localStorage.getItem(BSCAGREESTAKING)}>
          <ActivityInfoBox>
            ...
            <dl className="box">
              <dd className="item" dangerouslySetInnerHTML={{ __html: t('anyBscStakingTip31') }}></dd>
            ...
            {!localStorage.getItem(BSCAGREESTAKING) ? (
              <Flex>
                <Button2 style={{ height: '45px', maxWidth: '200px' }}>
                  <BackLink to={config.farmUrl}>{t('disagree')}</BackLink>
                </Button2>
                <Button1
                  onClick={() => {
